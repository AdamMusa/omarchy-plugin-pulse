...
class SuiteBackend
  ...
  def redact_http_url(value)
    ...
    scheme, remainder = url.split("://", 2)
    scheme = scheme.to_s.downcase
    return "" unless %w[http https].include?(scheme) && remainder
    authority = remainder.split("/", 2).first.to_s.split("@").last.to_s.downcase
    return "" if authority.empty? || authority.bytesize > 253
    ...
    if authority.start_with?("[")
      ...
      host = authority.byteslice(0, closing + 1).to_s
      suffix = authority.byteslice(closing + 1, authority.bytesize).to_s
      return "" unless host.bytes.all? { |byte| byte.between?(48, 57) || byte.between?(97, 102) || [46, 58, 91, 93].include?(byte) }
      port = suffix.byteslice(1, suffix.bytesize).to_s if suffix.start_with?(":")
      return "" unless suffix.empty? || suffix.start_with?(":")
    else
      pieces = authority.split(":")
      return "" if pieces.length > 2
      host = pieces.first.to_s
      port = pieces[1]
      valid_host = !host.empty? && !host.start_with?(".") && !host.end_with?(".") && !host.include?("..")
      host.each_byte do |byte|
        valid_host = false unless byte.between?(48, 57) || byte.between?(97, 122) || [45, 46].include?(byte)
      end
      return "" unless valid_host
    end
    if port
      valid_port = !port.empty?
      port.each_byte { |byte| valid_port = false unless byte.between?(48, 57) }
      return "" unless valid_port && port.to_i.between?(1, 65_535)
    end
    clean("#{scheme}://#{host}#{port ? ":#{port}" : ""}", 180)
  end
